...
# ---------------------------------------------------------------------------
# PID / file lock — protezione contro doppio avvio
# ---------------------------------------------------------------------------
import os, sys

_LOCK_FILE = Path(__file__).with_suffix(".lock")

def _acquire_lock() -> None:
    """Termina se un'altra istanza è già in esecuzione."""
    if _LOCK_FILE.exists():
        try:
            ...
            os.kill(pid, 0)
            print(
                f"[live_runner_btc4h] ERRORE: runner già in esecuzione (PID {pid}). "
                f"Se è un falso positivo, elimina {_LOCK_FILE} e riprova.",
                file=sys.stderr,
            )
            sys.exit(1)
        except (ProcessLookupError, PermissionError):
            pass  # processo morto, lock stale → sovrascriviamo
